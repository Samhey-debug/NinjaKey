...
export default async function handler(req, res) {
  const { name, username, password, dbpassword } = req.query;
  if (!name || !username || !password || !dbpassword) {
    return res.status(400).json({ message: 'Filename, username, password, and database password are required' });
  }
  ...
  try {
    ...
    const newContent = `${content}${username}:${password}\n`;
